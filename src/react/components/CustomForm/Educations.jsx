      ...
      <FieldArray name={userValueName}>
        {
          ({form}) => {
            ...
            const handleAdd = () => setFieldValue(userValueName, [...items, '']);
            ...
            return (
              items.map((item, index) => (
                  <div className='input-group-array' key={index}>
                    <Input type='text' label='Name' name={`${userValueName}[${index}].name`} {...props}/>
                    <Input type='text' label='School' name={`${userValueName}[${index}].school`} {...props}/>
